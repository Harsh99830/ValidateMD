...
# --------------------------------------------------------------------
# ✅ Wrapper Function for Integration with validate_provider.py
# --------------------------------------------------------------------
def validate_via_npi_registry(npi_number: str) -> dict:
    """
    Wrapper for real-time NPI registry validation.
    Fetches provider data for a single NPI and returns essential normalized fields.
    """
    try:
        provider_data = get_npi_provider_data(npi_number)
        if provider_data.get("status") == "SUCCESS":
            return {
                "npi": npi_number,
                "name": provider_data.get("full_name", "N/A"),
                "address": f"{provider_data.get('Address', 'N/A')}, "
                           f"{provider_data.get('City', '')}, "
                           f"{provider_data.get('State', '')}, "
                           f"{provider_data.get('Zip', '')}",
                "phone": provider_data.get("Phone", "N/A"),
                "specialty": provider_data.get("Speciality", "N/A")
            }
        else:
            return {
                "npi": npi_number,
        ...
        print(f"❌ Error validating NPI {npi_number}: {e}")
        return {
            "npi": npi_number,
            "name": "ERROR",
            "address": "ERROR",
